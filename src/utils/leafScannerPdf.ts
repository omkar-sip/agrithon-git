...
export function downloadLeafScanPdf(params: {
  ...
  const format = params.imageDataUrl.toLowerCase().includes('image/png') ? 'PNG' : 'JPEG'
  ...
  try {
    ...
    const imageWidth = contentW
    const maxImageHeight = 78
    ensureSpace(maxImageHeight + 18)
    doc.addImage(params.imageDataUrl, format, margin, y, imageWidth, maxImageHeight)
    y += maxImageHeight + 6
  } catch {
  ...
  params.analysis.treatments.forEach((treatment, index) => {
    const usageLines = doc.splitTextToSize(treatment.usage, contentW - TREATMENT_SIDE_PADDING * 2)
    ...
    doc.setFontSize(11)
    setTextColor(COLORS.text)
    doc.text(`${index + 1}. ${treatment.name}`, margin + TREATMENT_SIDE_PADDING, y + 8)

    const typeLabel = treatment.type.toUpperCase()
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(8)
    ...
    doc.setFontSize(9)
    setTextColor(COLORS.muted)
    doc.text(`Estimated cost: INR ${treatment.averageCostInr}`, margin + TREATMENT_SIDE_PADDING, y + 15)
